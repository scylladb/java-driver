...
public final class DeprecatedGraphConfig {
  ...
  public static Set<String> findConfiguredOptions(DriverConfig config) {
    ...
    for (DriverExecutionProfile profile : config.getProfiles().values()) {
      ...
      if (profile.isDefined(DefaultDriverOption.METRICS_SESSION_ENABLED)) {
        for (String path : profile.getStringList(DefaultDriverOption.METRICS_SESSION_ENABLED)) {
          if (isDeprecatedSessionMetric(path)) {
            result.add(DefaultDriverOption.METRICS_SESSION_ENABLED.getPath() + "=" + path);
          }
        }
      }
      if (profile.isDefined(DefaultDriverOption.METRICS_NODE_ENABLED)) {
        for (String path : profile.getStringList(DefaultDriverOption.METRICS_NODE_ENABLED)) {
