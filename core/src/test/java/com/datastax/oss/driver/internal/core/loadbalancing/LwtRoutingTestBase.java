...
public abstract class LwtRoutingTestBase extends LoadBalancingPolicyTestBase {
  ...
  @Test
  public void should_prioritize_preferred_remote_dcs_for_non_replicas() {
    // Set up 3 DCs: dc1 (local), dc2, dc3 with dc3 as preferred remote
    when(node4.getDatacenter()).thenReturn("dc2");
    when(node5.getDatacenter()).thenReturn("dc3");

    when(defaultProfile.getStringList(
            DefaultDriverOption.LOAD_BALANCING_DC_FAILOVER_PREFERRED_REMOTE_DCS))
        .thenReturn(ImmutableList.of("dc3"));

    BasicLoadBalancingPolicy preferredPolicy =
        createPolicy(context, DriverExecutionProfile.DEFAULT_NAME);
    preferredPolicy.init(
        ImmutableMap.of(
            UUID.randomUUID(), node1,
            UUID.randomUUID(), node2,
            UUID.randomUUID(), node3,
    ...
    given(request.getRoutingKey()).willReturn(ROUTING_KEY);
    given(request.getRequestRoutingType()).willReturn(RequestRoutingType.LWT);
    // All replicas in local DC, so non-replicas are only remote
    given(tokenMap.getReplicasList(KEYSPACE, null, ROUTING_KEY))
        .willReturn(ImmutableList.of(node1, node2, node3));

    Queue<Node> plan = preferredPolicy.newQueryPlan(request, session);

    assertThat(plan).hasSize(5);
    assertThat(plan.poll()).isEqualTo(node1); // local replica
    assertThat(plan.poll()).isEqualTo(node2); // local replica
    assertThat(plan.poll()).isEqualTo(node3); // local replica
    assertThat(plan.poll()).isEqualTo(node5); // dc3 non-replica (preferred remote)
    assertThat(plan.poll()).isEqualTo(node4); // dc2 non-replica
  }
}
