...
[Prepared statements]() allow Cassandra to cache parsed query strings
...
The driver caches prepared statements – see [CqlSession.prepare(SimpleStatement)](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/CqlSession.html#prepare-com.datastax.oss.driver.api.core.cql.SimpleStatement-) for the fine
...
you might want to experiment with different settings. See the [tuning]() section
in the connection pooling page.

### Compression

Consider [compression]() if your queries return large payloads; it might help to
...
[Request trackers]() are on the hot path (that is, invoked on I/O threads, each
...
Similarly, some of the driver’s [metrics]() are updated for every request (if the metric
is enabled).
...
[Throttling]() can help establish more predictable server performance, by controlling
...
The driver uses [CqlIdentifier](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/CqlIdentifier.html) to deal with [case sensitivity](). When you
...
require creating an identifier (the rules are detailed [here](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/data/AccessibleByName.html)).
...
[GenericType](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/type/reflect/GenericType.html) is used to express complex generic types – such as
[nested collections]() – in getters and setters. These objects are immutable
...
Similarly, [built queries]() are immutable and don’t need a reference to a live
...
The configuration API allows you to build [derived profiles]() at
runtime.
...
The driver maintains [metadata]() about the state of the Cassandra cluster. This work is
done on dedicated “admin” threads (see the [thread pooling]() section below), so it’s
...
Perhaps more interestingly, metadata can be [filtered]() to a specific
...
To get a sense of the time spent on metadata refreshes, enable [debug logs]() and look
...
[deactivate the metadata]() on all clients while you apply the
...
* apply your changes serially. The driver handles this automatically by checking for
  [schema agreement]() after each DDL query. Run them from the
...
* send all the changes to the same coordinator. This is one of the rare cases where we recommend
  using [Statement.setNode()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/cql/Statement.html#setNode-com.datastax.oss.driver.api.core.metadata.Node-).
...
[internal]() API (look at the `NettyOptions` interface). In particular, it is
