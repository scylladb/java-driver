...
* create the prepared statement with `session.prepare()`, call [bind()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/cql/PreparedStatement.html#bind-java.lang.Object...-) or
  [boundStatementBuilder()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/cql/PreparedStatement.html#boundStatementBuilder-java.lang.Object...-) on it to create executable
  statements.
...
* which bound variables are part of the partition key. This allows bound statements to automatically
  compute their [routing key]().
* more optimizations might get added in the future. For example, [CASSANDRA-10813](https://issues.apache.org/jira/browse/CASSANDRA-10813) suggests adding
  an “[idempotent]()” flag to the response.

If you have a unique query that is executed only once, a [simple statement]() will be more
...
[API docs](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/CqlSession.html#prepare-com.datastax.oss.driver.api.core.cql.SimpleStatement-).
...
`cql-prepared-cache-size`. The cache uses [weak values]([guava eviction](https://github.com/google/guava/wiki/CachesExplained#reference-based-eviction)) eviction, so this
...
[BoundStatement](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/cql/BoundStatement.html). As shown previously, there is a shorthand to provide the parameters in the same
call:
...
This can be ambiguous if the query uses the same column multiple times, like in `select * from sales where sku = ? and date > ? and date < ?`. In these situations, use positional setters or named
parameters.
...
Read the [reference configuration]() for a detailed description of each
of those options.
...
With Cassandra 4 and [native protocol]() v5, this issue is fixed
([CASSANDRA-10786](https://issues.apache.org/jira/browse/CASSANDRA-10786)): the server detects that the driver is operating on stale metadata and sends the
