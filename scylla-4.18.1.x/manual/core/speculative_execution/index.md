...
If a query is [not idempotent](), the driver will never schedule speculative
executions for it, because there is no way to guarantee that only one node will apply the mutation.

## Configuration

Speculative executions are controlled by a policy defined in the [configuration]().
...
Finally, you can create your own policy by implementing [SpeculativeExecutionPolicy](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/specex/SpeculativeExecutionPolicy.html), and
referencing your implementation class from the configuration.

## How speculative executions affect retries

Turning on speculative executions doesn’t change the driver’s [retry]() behavior. Each
...
monitoring the request percentiles with the `cql-requests` [metric](). Then use the
...
requests, identified by a unique number called *stream id* (see also the [pooling]()
...
The best way to monitor this is to compare the `pool.orphaned-streams` [metric]() to the
...
Note: ordering issues are only a problem with [server-side timestamps](), which
...
Or use a client-side [timestamp generator]().
