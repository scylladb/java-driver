...
[ExponentialReconnectionPolicy](https://github.com/scylladb/java-driver/blob/scylla-4.18.1.x/core/src/main/java/com/datastax/oss/driver/internal/core/connection/ExponentialReconnectionPolicy.java) is the default; it starts with a base delay, and then doubles it
after each attempt. [ConstantReconnectionPolicy](https://github.com/scylladb/java-driver/blob/scylla-4.18.1.x/core/src/main/java/com/datastax/oss/driver/internal/core/connection/ConstantReconnectionPolicy.java) uses the same delay every time, regardless of the
previous number of attempts.

You can also write your own policy; it must implement [ReconnectionPolicy](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/connection/ReconnectionPolicy.html) and declare a public
...
* [t = 2.5] the control connection tries to find a new node. It invokes the
  [load balancing policy]() to get a query plan, which happens to start with
