...
The basic component that encapsulates conversion logic is [MapperResultProducer](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/result/MapperResultProducer.html). Our DAO has two
...
1. `canProduce()` is used to select a producer. All registered producers are tried in the order that
   they were added, the first one that returns `true` is used. The [GenericType](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/type/reflect/GenericType.html) argument is a
...
2. `execute()` is invoked once the statement is ready to be sent. Note that the producer is not only
   ...
   this effect, it receives the [MapperContext](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/MapperContext.html), which provides access to the session. The
...
2. Whenever a return type references a mapped entity, the mapper processor will detect it and inject
   the corresponding [EntityHelper](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/entity/EntityHelper.html) in the `execute()` method. This is a general-purpose utility
...
[MapperResultProducerService](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/result/MapperResultProducerService.html):
