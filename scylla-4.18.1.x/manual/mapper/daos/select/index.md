...
Annotate a DAO method with [@Select](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html) to generate a query that selects one or more rows, and maps
them to [Entities]():
...
If the annotation doesn’t have a [customWhereClause()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#customWhereClause--), the mapper defaults to a selection by
primary key (partition key + clustering columns). The method’s parameters must match the types of
the [primary key columns](), in the exact order (as defined by
the [@PartitionKey](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/PartitionKey.html) and [@ClusteringColumn](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/ClusteringColumn.html) annotations). The parameter names don’t necessarily need
...
If the annotation has a [customWhereClause()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#customWhereClause--), it completely replaces the WHERE clause. The provided
...
The generated SELECT query can be further customized with [limit()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#limit--), [perPartitionLimit()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#perPartitionLimit--),
[orderBy()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#orderBy--), [groupBy()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#groupBy--) and [allowFiltering()](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Select.html#allowFiltering--). Some of these clauses can also contain placeholders
...
[@CqlName]()**. For example if the primary key is `((day int, hour int, minute int), ts timestamp)`:
...
* the entity class itself. If the query returns no rows, the method will return `null`. If it
  ...
  Product findById(UUID productId);
  ```
* an [Optional](https://docs.oracle.com/javase/8/docs/api/java/util/Optional.html) of the entity class. If the query returns no rows, the method will return
  `Optional.empty()`. If it returns more than one row, subsequent rows will be discarded.
  ```java
  @Select
  Optional<Product> findById(UUID productId);
  ```
* a [PagingIterable](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/PagingIterable.html) of the entity class. It behaves like a result set, except that each element is
  a mapped entity instead of a row.
  ```java
  @Select(customWhereClause = "description LIKE :searchString")
  PagingIterable<Product> findByDescription(String searchString);
  ```
* a [Stream](https://docs.oracle.com/javase/8/docs/api/java/util/stream/Stream.html) of the entity class. It behaves like a result set, except that each element is a mapped
  ...
  [PagingIterable.spliterator](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/PagingIterable.html#spliterator--).
  ```java
  @Select(customWhereClause = "description LIKE :searchString")
  Stream<Product> findByDescription(String searchString);
  ```
* a [CompletionStage](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CompletionStage.html) or [CompletableFuture](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CompletableFuture.html) of any of the above. The method will execute the query
  asynchronously. Note that for iterables, you need to switch to the asynchronous equivalent
  [MappedAsyncPagingIterable](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/MappedAsyncPagingIterable.html).
  ```java
  @Select
  ...
  are fetched from the server in the background. For this reason, *the usage of
  `CompletionStage<Stream<T>>` cannot be considered as a fully asynchronous execution method*.
* a [MappedReactiveResultSet](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/dse/driver/api/mapper/reactive/MappedReactiveResultSet.html) of the entity class.
  ```java
  @Select(customWhereClause = "description LIKE :searchString")
...
if the mapper was built from a session that has a [default keyspace](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/session/SessionBuilder.html#withKeyspace-com.datastax.oss.driver.api.core.CqlIdentifier-) set.

If a table was specified when creating the DAO, then the generated query targets that table.
Otherwise, it uses the default table name for the entity (which is determined by the name of the
entity class and the [naming strategy]()).
