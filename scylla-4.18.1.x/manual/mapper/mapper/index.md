...
Interface annotated with [@Mapper](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Mapper.html), entry point to mapper features.

* a corresponding builder gets generated (default: `[YourInterfacesName]Builder`).
* defines [@DaoFactory](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/DaoFactory.html) methods that provide DAO instances. They can be parameterized by keyspace
  and/or table.

---

The mapper interface is the top-level entry point to mapping features. It wraps a core driver
session, and acts as a factory of [DAO]() objects that will be used to execute requests.

It must be annotated with [@Mapper](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/Mapper.html):
...
more details on how to create a session, refer to the [core driver documentation]()).
...
methods annotated with [@DaoFactory](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/DaoFactory.html), that return a DAO interface:
...
[@DaoKeyspace](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/DaoKeyspace.html) and [@DaoTable](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/mapper/annotations/DaoTable.html) respectively, and be of type `String` or [CqlIdentifier](https://java-driver.docs.scylladb.com/scylla-4.18.1.x/api/com/datastax/oss/driver/api/core/CqlIdentifier.html):
...
* `dao1.findById` executes the query `SELECT ... FROM product WHERE id = ?`. No table name was
  ...
  the entity name converted with the default [naming strategy]()). No
...
corresponding name (according to the [naming strategy]()). It tries
...
* the types must be compatible, either according to the [default type
  mappings](), or via a [custom
  codec]() registered with the session.
