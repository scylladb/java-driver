...
libraries, and ships with bindings for three popular ones: [Dropwizard Metrics](https://metrics.dropwizard.io/4.1.2) , [Micrometer
Metrics](https://micrometer.io/docs) and [MicroProfile Metrics](https://github.com/eclipse/microprofile-metrics).
...
1. Add a dependency to `java-driver-metrics-micrometer` in your application. This separate driver
...
1. You should also exclude Dropwizard and HdrHistogram, which are two transitive dependencies of the
...
1. Add a dependency to `java-driver-metrics-microprofile` in your application. This separate driver
...
`advanced.metrics.factory.class` option. See the [reference configuration](../configuration/reference/).
...
To find out which metrics are available, see the [reference configuration](../configuration/reference/). It contains a
...
significant digits to use, and its refresh interval. Again, see the [reference configuration](../configuration/reference/) for
more details.
...
its [official documentation](https://micrometer.io/docs/registry/jmx).
...
CSV files, SLF4J logs and Graphite. Refer to their [manual](https://metrics.dropwizard.io/4.1.2/getting-started.html) for more details.
