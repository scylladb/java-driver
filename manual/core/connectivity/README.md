...
* contact points and a local datacenter, and nothing else, when every node is reachable at the
  ...
  public internet.
* a cloud private endpoint that publishes per-node routes needs client routes, which this version
  does not have: they are first available in driver 4.19.0.7, see [client routes](client_routes/).
...
| [A cloud private endpoint that publishes a per-node entry in `system.client_routes`](client_routes/) -- AWS PrivateLink, Azure Private Link, GCP Private Service Connect | client routes, which need driver 4.19.0.7 or later |
