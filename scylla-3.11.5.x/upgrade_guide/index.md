...
2. `ReadFailureException` and `WriteFailureException` are now surfaced to
   ...
   using `RetryDecision.rethrow()`, see [JAVA-1944](https://datastax-oss.atlassian.net/browse/JAVA-1944).

## 3.5.0

1. The `DowngradingConsistencyRetryPolicy` is now deprecated, see [JAVA-1752](https://datastax-oss.atlassian.net/browse/JAVA-1752).
   It will also be removed in the next major release of the driver (4.0.0),
   see [JAVA-1376](https://datastax-oss.atlassian.net/browse/JAVA-1376).
   ...
   `DowngradingConsistencyRetryPolicy`, see this [online example](https://github.com/datastax/java-driver/blob/3.x/driver-examples/src/main/java/com/datastax/driver/examples/retry/DowngradingRetry.java) that illustrates
   how to implement a downgrading logic at application level.

1. The `TokenAwarePolicy` now has a new constructor that takes a `ReplicaOrdering`
   argument, see [JAVA-1448](https://datastax-oss.atlassian.net/browse/JAVA-1448).
...
* otherwise, you might want to review how your code positions the `setIdempotent` flag on statements. In most cases the
  ...
  [query idempotence]() section of the manual.
...
7. `PoolingOptions#setMinSimultaneousRequestsPerConnectionThreshold(HostDistance, int)` has been removed. The new connection pool resizing algorithm introduced by
...
12. [Client-side timestamp generation]() is
    now the default (provided that [native
    protocol]() v3 or higher is in use). The
...
14. The following methods were added for [Custom payloads]():
...
27. `ColumnMetadata.getIndex()` has been removed.
    ...
    by the deleted method, *even for Cassandra versions <= 3.0*.
...
29. `SSLOptions` has been refactored to allow the option to choose between JDK and Netty-based
    ...
    the [SSL documentation]() for more details.
...
2. The following properties in `PoolingOptions` were renamed:
   ...
   * `MaxSimultaneousRequestsPerHostThreshold` to `MaxRequestsPerConnection`
   ...
   use multiple connections. See [this page]() for more
   information.
...
1. `SocketOptions#getTcpNoDelay()` is now TRUE by default (it was previously undefined).
...
2. Netty is not shaded anymore in the default Maven artifact. However we publish a
   [shaded artifact]() under a different classifier.
3. The internal initialization sequence of the Cluster object has been slightly changed:
