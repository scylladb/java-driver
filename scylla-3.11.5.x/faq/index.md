...
When using [native protocol]() version 2 or
...
The driver provides a convenience method [wasApplied](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/ResultSet.html#wasApplied--) to check this on the result set directly:
...
See [Blobs.java](https://github.com/datastax/java-driver/tree/3.11.5/driver-examples/src/main/java/com/datastax/driver/examples/datatypes/Blobs.java) in the `driver-examples` module for some examples and explanations.
...
One simple approach to remedying this is to use something like a [Semaphore](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/Semaphore.html) to limit the number of concurrent
`executeAsync` requests at a time.  Alternatively, one could submit requests X at a time and collect the returned
`ResultSetFuture`s from `executeAsync` and use [Futures.allAsList](https://google.github.io/guava/releases/19.0/api/docs/com/google/common/util/concurrent/Futures.html#allAsList(java.lang.Iterable)) and wait on completion of the resulting future
...
using the Logback logging framework, which provides the capability to include [packaging data](https://logback.qos.ch/reasonsToSwitch.html#packagingData) for classes
in stack traces.
...
Finally, some monitoring and agent-based tools such as [DynaTrace](https://www.dynatrace.com/) offer solutions that instrument the driver to
