...
* for other nodes, addresses will be discovered dynamically, either by inspecting `system.peers` on already connected
  nodes, or via push notifications received from the [control host]() when new nodes are discovered
  by gossip.
...
properties in [cassandra.yaml](https://docs.datastax.com/en/cassandra/3.x/cassandra/configuration/configCassandra_yaml.html):

* [rpc_address](https://docs.datastax.com/en/cassandra/3.x/cassandra/configuration/configCassandra_yaml.html?scroll=configCassandra_yaml__rpc_address) or [rpc_interface](https://docs.datastax.com/en/cassandra/3.x/cassandra/configuration/configCassandra_yaml.html?scroll=configCassandra_yaml__rpc_interface) is the address that the Cassandra process *binds to*. You must set one or the other,
  not both (for more details, see the inline comments in the default `cassandra.yaml` that came with your installation);
* [broadcast_rpc_address](https://docs.datastax.com/en/cassandra/3.x/cassandra/configuration/configCassandra_yaml.html?scroll=configCassandra_yaml__broadcast_rpc_address) (introduced in Cassandra 2.1) is the address to share with clients, if it is different than
...
that implements [AddressTranslator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/AddressTranslator.html) and register an instance with your `Cluster`:
...
[Address resolution](https://github.com/scylladb/java-driver/blob/scylla-4.x/manual/core/address_resolution/README.md#client-routes-cloud-private-endpoint-deployments).

A custom [AddressTranslator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/AddressTranslator.html) is not an equivalent. It receives an `InetSocketAddress` and translates
...
[Java Driver 4.x](https://github.com/scylladb/java-driver/tree/scylla-4.x).
...
The driver provides an address translator that does just that: [EC2MultiRegionAddressTranslator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/EC2MultiRegionAddressTranslator.html).
