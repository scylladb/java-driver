...
Use [PreparedStatement](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/PreparedStatement.html) for queries that are executed multiple times in your application:
...
To turn the statement into its executable form, you need to *bind* it to create a [BoundStatement](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/BoundStatement.html). As shown previously,
...
unset, and the server rejected the request with `Unexpected unset value for bind variable 1`. This driver is more forgiving, but only on some of
...
mentions the same column more than once, for example: `select * from sales where sku = ? and date > ? and date < ?`, where both range markers are
...
* [setPrepareOnAllHosts](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/QueryOptions.html#setPrepareOnAllHosts-boolean-) controls whether statements are initially
  re-prepared on other hosts (step 1 above);
* [setReprepareOnUp](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/QueryOptions.html#setReprepareOnUp-boolean-) controls whether statements are re-prepared on a
...
[CASSANDRA-10786](https://issues.apache.org/jira/browse/CASSANDRA-10786) and [JAVA-1196](https://datastax-oss.atlassian.net/browse/JAVA-1196) for more information.
