...
A Cassandra cluster is typically composed of multiple hosts; the [LoadBalancingPolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LoadBalancingPolicy.html) (sometimes abbreviated LBP) is a
...
[datacenter-aware](), [token-aware]() policy:
...
For each host, the policy computes a **[distance](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/HostDistance.html)** that determines how the driver will establish connections
to it:

* `LOCAL` and `REMOTE` are “active” distances, meaning that the driver will keep open connections to the host. They
  differ in the number of connections opened, depending on your [pooling options](). Also, the
  [control connection]() will  favor local nodes if possible.
...
startup or if a node was down and just came back up); you can also trigger it with [refreshConnectedHosts](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/PoolingOptions.html#refreshConnectedHosts--):
...
driver will then try each host in sequence, according to the [retry policy]() and
...
## [RoundRobinPolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/RoundRobinPolicy.html)
...
## [DCAwareRoundRobinPolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/DCAwareRoundRobinPolicy.html)
...
datacenter of the first contact point that was reached [at initialization](). However,
...
## [RackAwareRoundRobinPolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/RackAwareRoundRobinPolicy.html)
...
You can also leave it out, and the driver will use the datacenter and rack of the first contact point that was reached [at initialization]().
...
## [TokenAwarePolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/TokenAwarePolicy.html)
...
by default, unless it’s been explicitly disabled by [QueryOptions#setMetadataEnabled](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/QueryOptions.html#setMetadataEnabled-boolean-).

Then you need to consider whether routing information (provided by [Statement#getKeyspace](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/Statement.html#getKeyspace--) and
[Statement#getRoutingKey](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/Statement.html#getRoutingKey--)) can be computed automatically for your statements; if not, you may provide it yourself (if a
...
For [built statements](), the keyspace is available if it was provided while building the query; the
...
For [bound statements](), the keyspace is always available; the routing key is only available if
...
For [batch statements](), the routing information of each child statement is inspected; the first
...
## [LatencyAwarePolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LatencyAwarePolicy.html)
...
* the [exclusion threshold](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LatencyAwarePolicy.Builder.html#withExclusionThreshold-double-) controls how much worse a host must perform (compared to the fastest
...
* since a host’s performance can vary over time, its score is computed with a time-weighted average; the
  [scale](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LatencyAwarePolicy.Builder.html#withScale-long-java.util.concurrent.TimeUnit-) controls how fast the weight given to older latencies decreases over time;
* the [retry period](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LatencyAwarePolicy.Builder.html#withRetryPeriod-long-java.util.concurrent.TimeUnit-) is the duration for which a slow host will be penalized;
* the [update rate](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LatencyAwarePolicy.Builder.html#withUpdateRate-long-java.util.concurrent.TimeUnit-) defines how often the minimum average latency (i.e. the fastest host) is recomputed;
* the [minimum measurements](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/LatencyAwarePolicy.Builder.html#withMininumMeasurements-int-) threshold guarantees that we have enough measurements before we
...
[WhiteListPolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/WhiteListPolicy.html) wraps another policy with a white list, to ensure that the driver will only ever connect to a
...
[HostFilterPolicy](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/policies/HostFilterPolicy.html) is a generalization of that concept, where you provide the predicate that will determine if a host is
included or not.
