...
The [native protocol]() version 4 introduces a new feature called [Custom Payloads](https://issues.apache.org/jira/browse/CASSANDRA-8553).

According to the [protocol V4 specification](https://github.com/apache/cassandra/blob/trunk/doc/native_protocol_v4.spec), custom payloads are generic key-value maps
...
*Please note that this is an advanced feature that requires specific server-side configuration (see below).*

## Enabling custom payloads on C\* nodes

What the server is supposed to do with a payload depends on the [QueryHandler](https://issues.apache.org/jira/browse/CASSANDRA-6659) used server-side
...
an [UnsupportedFeatureException](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/exceptions/UnsupportedFeatureException.html) (wrapped in a [NoHostAvailableException](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/exceptions/NoHostAvailableException.html))
...
1. Inspect the current negotiated protocol version, and only send payloads if version is equal to or
   greater than 4:
...
1. From a `PreparedStatement`: to retrieve the payload that the server sent back
