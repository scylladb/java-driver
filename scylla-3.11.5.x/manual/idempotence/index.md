...
Idempotence matters for [retries]() and [speculative query executions](). The driver
will bypass those features if the [Statement#isIdempotent()](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/Statement.html#isIdempotent--) flag is set to `false`, to ensure that the
...
The [QueryBuilder](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/querybuilder/QueryBuilder.html) DSL tries to infer the `isIdempotent` flag on the statements it generates. The following statements
...
One important aspect of lightweight transactions is [linearizability](https://en.wikipedia.org/wiki/Linearizability#Definition_of_linearizability): given a set of concurrent operations on a column
