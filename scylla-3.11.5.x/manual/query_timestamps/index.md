...
of Cassandra that supports [native protocol]() v3 or
above.

The driver will use a [TimestampGenerator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/TimestampGenerator.html) to generate a timestamp for each query, and send it
...
1. [AtomicMonotonicTimestampGenerator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/AtomicMonotonicTimestampGenerator.html), which guarantees monotonicity of timestamps for all
   threads. This is the default implementation (i.e. what will be used if you don’t call
   `withTimestampGenerator`);
2. [ThreadLocalMonotonicTimestampGenerator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/ThreadLocalMonotonicTimestampGenerator.html), which guarantees per-thread monotonicity of timestamps.
...
On some systems, however, it is possible to have a better granularity by using a [JNR](https://github.com/jnr/jnr-ffi)
call to [gettimeofday](http://man7.org/linux/man-pages/man2/settimeofday.2.html). This native call will be used when available, unless the system
...
* [configure your logging framework]() to use level `INFO` for the category
...
within the same microsecond, or in the event of a system clock skew, *both implementations might
...
server-side timestamps with protocol v3 or above, using [ServerSideTimestampGenerator](https://java-driver.docs.scylladb.com/scylla-3.11.5.x/api/com/datastax/driver/core/ServerSideTimestampGenerator.html):
