...
With the [protocol layer]() in place, the next step is to build the logic for a
...
pooling]()). `StreamIdGenerator` is not thread-safe.
...
The `advanced.netty` section in the [configuration]() exposes a
few high-level options.

For more elaborate customizations, you can [extend the
context]() to plug in a custom `NettyOptions`
...
The [user-facing API]() (`advanced.ssl-engine-factory` in the configuration, or
...
* implement `SslHandlerFactory`. Typically:
  * the constructor will create a Netty [SslContext](https://netty.io/4.1/api/io/netty/handler/ssl/SslContext.html) with [SslContextBuilder.forClient](https://netty.io/4.1/api/io/netty/handler/ssl/SslContextBuilder.html#forClient--), and store
    it in a field;
  * `newSslHandler` will delegate to one of the [SslContext.newHandler](https://netty.io/4.1/api/io/netty/handler/ssl/SslContext.html#newHandler-io.netty.buffer.ByteBufAllocator-) methods;
* [extend the context]() and override
