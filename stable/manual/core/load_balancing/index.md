...
[retry policy]() and [speculative execution policy]().
...
You can still write a [custom implementation]() if you have special
requirements.
...
To check which datacenters are defined in a given cluster, you can run [`nodetool status`](https://docs.scylladb.com/stable/operating-scylla/nodetool-commands/status.html). It will
...
can also use [cqlsh](https://docs.scylladb.com/stable/cql/cqlsh.html); if cqlsh is co-located too in the same datacenter, simply run the command
below:
...
- The driver will then attempt to open connections to those nodes. The actual number of connections
  to open to each one of those nodes is configurable, see [Connection pools]() for
...
1. **Application-level failover**: instead of letting the driver do the failover, implement the
   ...
   our [application-level failover example](https://github.com/scylladb/java-driver/blob/scylla-4.x/examples/src/main/java/com/datastax/oss/driver/examples/failover/CrossDatacenterFailover.java) for a good starting point.
2. **Infrastructure-level failover**: in this scenario, the failover is handled by the
   ...
   requires a purpose-built infrastructure. To help you explore this option, read our [white paper](https://www.datastax.com/sites/default/files/content/whitepaper/files/2019-09/Designing-Fault-Tolerant-Applications-DataStax.pdf).
...
* a keyspace: if you use a [per-query keyspace](), then it will be
  used for routing as well. Otherwise, the driver relies on [getRoutingKeyspace()](https://java-driver.docs.scylladb.com/scylla-4.19.0.x/api/com/datastax/oss/driver/api/core/session/Request.html#getRoutingKeyspace--);
* a routing key: it can be provided either by [getRoutingKey()](https://java-driver.docs.scylladb.com/scylla-4.19.0.x/api/com/datastax/oss/driver/api/core/session/Request.html#getRoutingKey--) (raw binary data) or
  [getRoutingToken()](https://java-driver.docs.scylladb.com/scylla-4.19.0.x/api/com/datastax/oss/driver/api/core/session/Request.html#getRoutingToken--) (already hashed as a token).
...
For [simple statements](), routing information is never computed
automatically:
...
For [bound statements](), the keyspace is always available; the routing key
...
For [batch statements](), the routing information of each child statement is
...
The node distance evaluator class must implement [NodeDistanceEvaluator](https://java-driver.docs.scylladb.com/scylla-4.19.0.x/api/com/datastax/oss/driver/api/core/loadbalancing/NodeDistanceEvaluator.html), and have a public
constructor that takes a [DriverContext](https://java-driver.docs.scylladb.com/scylla-4.19.0.x/api/com/datastax/oss/driver/api/core/context/DriverContext.html) argument: `public MyNodeDistanceEvaluator(DriverContext context)`.
...
Study the [LoadBalancingPolicy](https://java-driver.docs.scylladb.com/scylla-4.19.0.x/api/com/datastax/oss/driver/api/core/loadbalancing/LoadBalancingPolicy.html) interface and the built-in [BasicLoadingBalancingPolicy] for the
