...
used exclusively for administrative requests. It shows up in [Node.getOpenConnections](https://java-driver.docs.scylladb.com/scylla-4.19.2.x/api/com/datastax/oss/driver/api/core/metadata/Node.html#getOpenConnections--), as well as
the `pool.open-connections` [metric](); for example, if you’ve configured a pool size of
2, the control node will show 3 connections.

There are a few options to fine tune the control connection behavior in the
`advanced.control-connection` and `advanced.metadata` sections; see the [metadata]()
