...
* explanations about [driver dependencies]() and when they can be manually
  excluded.

Note: guidelines to build a GraalVM native image can be found [here]().
...
<tr>
  ...
  <td>
    ...
    href="https://github.com/scylladb/java-driver/blob/scylla-4.x/CONTRIBUTING.md#integration-tests">contribution
    guidelines</a> and <a
    href="https://github.com/scylladb/java-driver/tree/scylla-4.x/integration-tests">internal tests</a> for
    guidance.
  </td>
</tr>
</table>
...
We publish the driver to [Maven central](https://search.maven.org/#search%7Cga%7C1%7Ccom.datastax.oss). Most modern build tools can download the
...
`pom.xml` is the [Project Object Model](https://maven.apache.org/guides/introduction/introduction-to-the-pom.html) that describes your application. We declare the
...
`Main.java` is the canonical example introduced in our [quick start](); it connects
...
[CqlSession.builder().addContactPoint()](https://java-driver.docs.scylladb.com/scylla-4.19.2.x/api/com/datastax/oss/driver/api/core/session/SessionBuilder.html#addContactPoint-java.net.InetSocketAddress-) to use a different
address).
...
[Initialize a new project](https://guides.gradle.org/creating-new-gradle-builds/) with Gradle.
...
Then place [application.conf](), [logback.xml]() and
[Main.java]() in the same locations, and with the same contents, as in the Maven example:
...
[DataStax download server](http://downloads.datastax.com/java-driver/).
...
It is a required dependency, but we provide a a [shaded JAR]() that relocates it to a
...
[completely overridden]() with your own implementation,
...
* to get the process ID when generating [UUIDs](https://java-driver.docs.scylladb.com/scylla-4.19.2.x/api/com/datastax/oss/driver/api/core/uuid/Uuids.html).
...
enable compression. See the [Compression]() page for more details.

### Metrics

The driver exposes [metrics]() through the
...
if you are using a different metrics library, and you never call [Session.getMetrics](https://java-driver.docs.scylladb.com/scylla-4.19.2.x/api/com/datastax/oss/driver/api/core/session/Session.html#getMetrics--) anywhere in
...
explicitly is not required anymore if you don’t use it. *If you do use the graph API though, you now
need to explicitly include the dependencies below in your application*:
...
processing](https://docs.oracle.com/javase/8/docs/technotes/tools/windows/javac.html#sthref65) in your build, the Java compiler scans the entire classpath – including the driver’s
...
* the [SLF4J](https://www.slf4j.org/) API for [logging]().
