...
The [Netty pipeline]() gives us the ability to send low-level protocol messages on
a single connection.
...
it’s better to do it at the session level with a [throttler](), which provides
...
[RequestProcessorIT](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/core/session/RequestProcessorIT.java). It shows how you can build a session that returns Guava’s `ListenableFuture`
instead of Java’s `CompletionStage` (existing request type, different return type).

[GuavaDriverContext](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/example/guava/internal/GuavaDriverContext.java) is the custom context subclass. It plugs a custom registry that wraps the
default async processors with [GuavaRequestAsyncProcessor](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/example/guava/internal/GuavaRequestAsyncProcessor.java), to transform the returned futures.
...
For example, in the [RequestProcessorIT](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/core/session/RequestProcessorIT.java) example mentioned in the previous section, we remove the
ability to return `CompletionStage`, but add the ability to return `ListenableFuture`. Therefore we
expose a custom [GuavaSession](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/example/guava/api/GuavaSession.java) with a different return type for async methods:
...
`Session`. The driver provides `SessionWrapper` to that effect. See [DefaultGuavaSession](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/example/guava/internal/DefaultGuavaSession.java):
...
override to plug our wrapper. See [GuavaSessionBuilder](https://github.com/scylladb/java-driver/blob/scylla-4.x/integration-tests/src/test/java/com/datastax/oss/driver/example/guava/api/GuavaSessionBuilder.java):
