...
Starting with version 4.13.0, the driver ships with [embedded GraalVM configuration files](https://www.graalvm.org/reference-manual/native-image/BuildConfiguration/#embedding-a-configuration-file) that
...
1. When invoking the native image builder, add a `-H:ReflectionConfigurationFiles=reflection.json`
...
`reference.conf`, `application.conf`, `application.json`, `application.properties`. *These classpath
resources are all automatically included in the native image: you should not need to do it
manually*. See [Accessing Resources in Native Images](https://www.graalvm.org/latest/reference-manual/native-image/metadata/#resources) for more information on how classpath
resources are handled in native images.

## Configuring the logging backend

When configuring [logging](), the choice of a backend must be considered carefully, as
...
The [reactive execution model]() is compatible with GraalVM native images, but the
...
[Jackson](https://github.com/FasterXML/jackson) is used in [a few places]() in
...
When using [compression](), only LZ4 can be enabled in native images. **Snappy
...
The driver performs a few [native calls]() using
...
DSE [Geospatial types]() are supported on GraalVM native images; the following
...
1. When invoking the native image builder, add the following flags:
