...
You don’t need to manage connections yourself. You simply interact with a [CqlSession](https://java-driver.docs.scylladb.com/scylla-4.19.2.x/api/com/datastax/oss/driver/api/core/CqlSession.html) object, which
takes care of it.

**For a given session, there is one connection pool per connected node** (a node is connected when
it is up and not ignored by the [load balancing policy]()).
...
Pool sizes are defined in the `connection` section of the [configuration](). Here
...
[Tuning]() section at the end of this page.
...
The driver exposes node-level [metrics]() to monitor your pools (note that all metrics
...
* `pool.open-connections`: if this doesn’t match your configured pool size, something is preventing
  ...
  see [CASSANDRA-8086](https://issues.apache.org/jira/browse/CASSANDRA-8086));
...
driver, a [throttler]() is a better solution.
